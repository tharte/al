...
** What if we /only/ had functions? (and no /names/...)
- Mathematics
...
** What if we /only/ had functions? (and no /names/, and no /numerals/...)
- Mathematics
...
- \onslide<7-> simplest example of computation: the identity
    ...
    \onslide<9->
    $$\lambda\underbrace{1}_{\mbox{\scriptsize parameter}}. \underbrace{1}_{\mbox{\scriptsize body}}$$
