...
** Church Numerals
\begin{eqnarray*}
    {\tt C0} &=& C_0\bydefn \lambda f.\lambda x. x \alphaeq \lambda x.\lambda y.y = {\tt false} \\[2mm]
\pause
    {\tt C1} &=& C_1\bydefn \lambda f.\lambda x. f(x) \\[2mm]
\pause
    {\tt C2} &=& C_2\bydefn \lambda f.\lambda x. f(f(x)) \\[2mm]
\pause
    {\tt C3} &=& C_3\bydefn \lambda f.\lambda x. f(f(f(x))) \\[2mm]
\pause
             &\vdots&  \\[2mm]
\pause
             &=& C_N\bydefn \lambda f.\lambda x. f^N(x) 
\end{eqnarray*}
