...
\vspace{-0.5cm}
\begin{eqnarray*}
    \underbrace{(\lambda f.\lambda x. x)}_{\mbox{\scriptsize $C_0$}}\underbrace{(\lambda x.x+1)}_f \underbrace{(0)}_x & \betared & 0 \\[2mm]
\pause
    \underbrace{(\lambda f.\lambda x. f(x))}_{\mbox{\scriptsize $C_1$}}(\lambda x.x+1)(0) & \betared & \underbrace{(\lambda x.x+1)}_f\underbrace{(0)}_x \\
\pause
     & \betared & 1 \\[2mm]
\pause
    \bigl(\underbrace{(\lambda f.\lambda x. f(f(x)))}_{\mbox{\scriptsize $C_2$}}(\lambda x.x+1)\bigr)(0) & \betared & \underbrace{(\lambda x.x+1)}_f\underbrace{(\lambda x.x+1)}_f\underbrace{(0)}_x \\
\pause
...
- successor
    ...
    $$ {\tt succ} \bydefn \lambda n.\lambda f.\lambda x. f(nfx) = {\tt succ} \bydefn \lambda nfx.f((nf)x)$$
...
- predecessor
  ...
  \begin{eqnarray*}
    {\tt pred} &\bydefn& \lambda n.\lambda n(\lambda p. \lambda z.z({\tt succ}(p(T)))(p(T)))(\lambda z.z(C_0)(C_0))(F) \\
                     &=& \lambda n.n\lambda p.\lambda z.({\tt succ}(p(T)))(p(T))(\lambda z.z(C0)(C0))(F)
  \end{eqnarray*}
...
\pause
    #+begin_src R :session :exports both :results output :eval no
    `zerop?`(false) |> .to.logical()
    `zerop?`(true) |> .to.logical()
    `zerop?`(C0) |> .to.logical()
    `zerop?`(C3) |> .to.logical()
    #+end_src
    ...
    : function (y) 
    : y
    : <bytecode: 0x55fc444953a0>
    : <environment: 0x55fc44f2a238>
    : [1] TRUE
    : [1] FALSE
    #+LATEX: }
